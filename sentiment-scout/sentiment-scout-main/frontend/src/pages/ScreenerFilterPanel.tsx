      ...
      <div className="p-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        {show('sentiment') && (
          <>
            ...
            {/* Thresholds on our own computed columns (applied in-app on the
                merged rows — Finviz can't filter these). Null rows are excluded. */}
            <FilterSelect label="News Sent ≥ (3d)" k="min_news_sent" value={filters.min_news_sent} onChange={v => setFilter('min_news_sent', v)}
              options={[
                { value: '-0.2', label: '≥ -0.20' },
                { value: '0', label: '≥ 0.00' },
                { value: '0.1', label: '≥ 0.10' },
                { value: '0.2', label: '≥ 0.20 (bullish)' },
                { value: '0.5', label: '≥ 0.50' },
              ]} />
            <FilterSelect label="ST Social Sent ≥" k="min_social_sent" value={filters.min_social_sent} onChange={v => setFilter('min_social_sent', v)}
              options={[
                { value: '0', label: '≥ 0.00' },
                { value: '0.2', label: '≥ 0.20 (bullish)' },
                { value: '0.5', label: '≥ 0.50' },
            ...
            <FilterSelect label="ST Density ≥" k="min_density" value={filters.min_density} onChange={v => setFilter('min_density', v)}
              options={[
                { value: '5', label: '5+' },
                { value: '10', label: '10+' },
                { value: '25', label: '25+' },
