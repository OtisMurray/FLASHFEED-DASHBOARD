  ...
  const renderCell = (key: string) => {
    switch (key) {
      ...
      case 'rel_volume': {
        ...
        return <span className={clsx('font-mono', rv != null && rv >= 1.5 ? 'text-emerald-400' : 'text-neutral')}>{rv != null ? `${rv.toFixed(2)}x` : '—'}</span>
      }
      case 'rsi': {
        const rsi = (row as any).rsi
        return <span className={clsx('font-mono', rsi != null && rsi >= 70 ? 'text-red-400' : rsi != null && rsi <= 30 ? 'text-emerald-400' : 'text-neutral')}>{rsi != null ? rsi.toFixed(1) : '—'}</span>
      }
      case 'cap_tier': {
        const ct = (row as any).cap_tier
        return <span className="text-[10px] uppercase bg-slate-700 text-neutral px-1.5 py-0.5 rounded">{ct ?? '—'}</span>
      }
      case 'market_cap':
      ...
      case 'news_sentiment': {
        const v = row.news_sentiment
        if (v == null) return <span className="font-mono text-slate-600">—</span>
        return (
          <span className={clsx('font-mono', v >= 0.2 ? 'text-emerald-400' : v <= -0.2 ? 'text-red-400' : 'text-neutral')}
                title={`${row.news_article_count_3d ?? 0} scored articles (3d)`}>
            {v.toFixed(2)}
          </span>
        )
      }
      case 'stocktwits_sentiment': {
        const v = row.stocktwits_sentiment
        if (v == null) return <span className="font-mono text-slate-600">—</span>
        return (
          <span className={clsx('font-mono', v >= 0.2 ? 'text-emerald-400' : v <= -0.2 ? 'text-red-400' : 'text-neutral')}>
            {v.toFixed(2)}
      ...
      case 'stocktwits_density': {
        const v = row.stocktwits_density
        if (v == null) return <span className="font-mono text-slate-600">—</span>
        return <span className={clsx('font-mono', v >= 50 ? 'text-accent' : 'text-neutral')}>{v.toLocaleString()}</span>
      }
      case 'message_count':
