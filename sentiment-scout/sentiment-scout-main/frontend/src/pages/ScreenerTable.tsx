...
export function ScreenerTable({ rows, isLoading, viewMode, extraColumns = [] }: Props) {
  const base = COLUMNS[viewMode]
  // User-toggled columns append to the active view, deduped so a column already
  // shown by the view isn't doubled.
  const baseKeys = new Set(base.map(c => c.key))
  const columns = [...base, ...extraColumns.filter(c => !baseKeys.has(c.key))]
