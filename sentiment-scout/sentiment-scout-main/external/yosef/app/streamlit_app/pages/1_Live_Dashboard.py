...
Main Streamlit page for the IST495 Market Intelligence Dashboard.

This page:
- loads and displays aggregated Stocktwits sentiment data from MongoDB
- lets the user choose time windows, filters, and sorting options
- allows Finviz screener data to be loaded by URL/token or manual CSV
- adds one active rumor signal per ticker
- lets the user open a selected ticker in the Ticker Detail page

Only comments were added here; dashboard logic is unchanged.
"""
import sys
...
from finviz_elite_fetch import ( #type: ignore
    fetch_and_store_finviz_from_inputs,
    load_manual_finviz_csv_to_mongo,
)
...
# Custom CSS controls the dashboard look: dark background, colored metric cards,
# hero banner, table notes, ticker cards, and animated fade-in effects.
st.markdown(
    """
    <style>
    .stApp {
...
st.markdown(
    ...
    <div class="hero-card fade">
        ...
        <div class="hero-subtitle">
            Real-time social sentiment, one active rumor per ticker, message density, market context, and Finviz screener control
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
# MongoCfg stores the MongoDB connection settings used by the backend functions.
cfg = MongoCfg()

# =========================
# FINVIZ CONTROL PANEL
# =========================
st.markdown('<div class="section-title">📊 Finviz Screener Loader</div>', unsafe_allow_html=True)

with st.expander("Open Finviz URL / Token Loader", expanded=False):
    st.markdown(
        """
        Use this section when the user/client wants to enter their own Finviz screener URL and token.
        If Finviz returns HTML instead of CSV, use the manual CSV path fallback.
        """
    )

    with st.form("finviz_loader_form", clear_on_submit=False):
        finviz_url = st.text_input(
            "Finviz Screener URL",
            placeholder="https://elite.finviz.com/screener.ashx?v=152&f=sh_relvol_o5,ta_change_u3&ft=3&o=-change&ar=10",
        )

        finviz_token = st.text_input(
            "Finviz Token / auth",
            placeholder="Paste Finviz auth token here",
            type="password",
        )

        fetch_clicked = st.form_submit_button("🚀 Fetch Finviz Data from URL + Token")

    if fetch_clicked:
        try:
            result = fetch_and_store_finviz_from_inputs(
                screener_url=finviz_url,
                token=finviz_token,
            )

            st.success(
                f"Finviz data loaded successfully: {result.get('rows_inserted', 0)} rows inserted."
            )
            st.caption(f"Latest CSV: {result.get('latest_csv', '')}")
            st.caption(f"Dated CSV: {result.get('dated_csv', '')}")

        except Exception as e:
            st.error(f"Finviz direct fetch failed: {e}")
            st.warning(
                "Finviz may have returned HTML instead of CSV. Use the manual CSV fallback below."
            )

    st.markdown("---")
    st.markdown("#### Manual CSV fallback")

    with st.form("manual_csv_loader_form", clear_on_submit=False):
        manual_csv_path = st.text_input(
            "Manual Finviz CSV Path",
            placeholder=r"C:\Users\yosef\OneDrive\Desktop\Research Internship IST495\finviz_daily\finviz_2026_04_20.csv",
        )

        manual_clicked = st.form_submit_button("📁 Load Manual CSV to Mongo")

    if manual_clicked:
        try:
            result = load_manual_finviz_csv_to_mongo(manual_csv_path)
            st.success(
                f"Manual CSV loaded successfully: {result.get('rows_inserted', 0)} rows inserted."
            )
            st.caption(f"CSV path: {result.get('manual_csv', '')}")

        except Exception as e:
            st.error(f"Manual CSV load failed: {e}")
...
# These values are used when the page first loads or when the user resets controls.
DEFAULTS = dict(
    window_type="Last N",
    preset="30m",
    minutes=30,
    rolling_unit="minutes",
...
# Sidebar form collects all user controls first, then applies them together.
# This avoids rerunning the dashboard after every single small input change.
with st.sidebar.form("controls_form", clear_on_submit=False):
    st.markdown("### Dashboard Window")
    ...
    rolling_presets = ["1m", "3m", "5m", "10m", "30m", "1h", "4h", "1D", "1W"]

    preset = st.selectbox(
        "Rolling window",
        rolling_presets,
        index=rolling_presets.index(applied["preset"]) if applied["preset"] in rolling_presets else rolling_presets.index("30m"),
    ...
    _, window_unit_default = preset_to_window.get(applied["preset"], (30, "minutes"))
    ...
    rolling_unit = st.selectbox(
        "Custom rolling unit",
        ["minutes", "hours", "days", "weeks"],
        index=["minutes", "hours", "days", "weeks"].index(applied.get("rolling_unit", window_unit_default)),
        disabled=(window_type != "Last N"),
    )
...
# Save the selected filter settings into Streamlit session state.
# Session state keeps the user's settings after the app reruns.
if apply_clicked:
    chosen_value, chosen_unit = preset_to_window.get(preset, (30, "minutes"))

    st.session_state["filters"] = dict(
        window_type=window_type,
        preset=preset,
        minutes=int(minutes if window_type == "Last N" else chosen_value),
        rolling_unit=rolling_unit if window_type == "Last N" else chosen_unit,
...
try:
    if applied["window_type"] == "Last N":
        start_utc, end_utc = parse_window(
            "last_n",
            last_n=int(applied["minutes"]),
            unit=applied.get("rolling_unit", "minutes"),
        )
...
# Load dashboard data
# =========================
# Query MongoDB for ticker-level summary statistics in the selected time window.
# This includes post counts, sentiment score, source counts, and density.
df = agg_ticker_summary(cfg, start_utc, end_utc)
# Load the latest Finviz screener snapshot and merge market context
# such as relative volume, price change, and volume into the dashboard table.
...
if not df.empty:
# Add one active rumor per ticker.
# The backend chooses a message that may influence users to buy in or leave.
    rumor_df = get_active_rumors_for_tickers(
        cfg,
        df["stream_symbol"].astype(str).tolist(),
        start_utc,
        end_utc,
    )
...
# Summary metrics
# =========================
# Build top-level dashboard metrics shown in the colored summary cards.
...
with c1:
    st.markdown(
        f'<div class="mini-card mini-blue fade"><div class="mini-label">Tickers in View</div><div class="mini-value">{total_tickers}</div></div>',
        unsafe_allow_html=True,
    )
with c2:
    st.markdown(
        f'<div class="mini-card mini-purple fade"><div class="mini-label">Total Clean Posts</div><div class="mini-value">{total_posts}</div></div>',
        unsafe_allow_html=True,
    )
with c3:
    st.markdown(
        f'<div class="mini-card mini-red fade"><div class="mini-label">Tickers With Active Rumor</div><div class="mini-value">{tickers_with_active_rumor}</div></div>',
        unsafe_allow_html=True,
    )
with c4:
    st.markdown(
        f'<div class="mini-card mini-green fade"><div class="mini-label">Traditional Posts</div><div class="mini-value">{total_traditional}</div></div>',
        unsafe_allow_html=True,
    )
with c5:
    st.markdown(
        f'<div class="mini-card mini-orange fade"><div class="mini-label">Average Sentiment</div><div class="mini-value">{avg_sentiment:.3f}</div></div>',
        unsafe_allow_html=True,
    )
# Cleaner display names for tables so dashboard columns are easier to read.
display_name_map = {
...
# Featured ticker cards highlight the most active tickers and provide
# a quick button to open each ticker's detail page.
    ...
    for i, (_, row) in enumerate(top_cards.iterrows()):
        ...
        ticker_val = str(row.get("stream_symbol", "")).strip().upper()

        col.markdown(
            f"""
            <div class="ticker-card fade" style="border-left: 4px solid {border};">
                <div class="ticker-name">{ticker_val or 'N/A'}</div>
        ...
        if col.button(f"Open {ticker_val}", key=f"open_card_{ticker_val}_{i}"):
...
# Simple rule-based insight text summarizes overall market sentiment,
# rumor activity, and whether discussion is more social or traditional.
insights = []
...
# Signal panels split the dashboard into three views:
# active rumors, traditional-source coverage, and social/rumor-social activity.
...
with top_a:
    ...
    if df.empty or "active_rumor" not in df.columns:
        ...
        rumor_panel = df[df["active_rumor"].fillna("") != ""].copy().head(10)
        if rumor_panel.empty:
            st.info("No active rumor tickers available")
        else:
            show = ["stream_symbol", "rumor_direction", "rumor_time_label", "active_rumor"]
            st.dataframe(
                rumor_panel[show].rename(columns=display_name_map),
...
with top_b:
    ...
    if df.empty or "traditional_posts" not in df.columns:
        ...
        st.dataframe(
            trad_df[trad_show].rename(columns=display_name_map),
            use_container_width=True,
            hide_index=True,
        )
...
with top_c:
    ...
    if df.empty or "social_posts" not in df.columns:
        ...
        st.dataframe(
            social_df[social_show].rename(columns=display_name_map),
            use_container_width=True,
            hide_index=True,
        )
...
# Main table + 3D plot
# =========================
# Main layout: ticker table on the left and 3D market visualization on the right.
...
# Interactive table lets the user select a ticker and navigate to its detail page.
        ...
        st.markdown(
            '<div class="table-note">Use the Open checkbox to jump into that ticker detail page.</div>',
            unsafe_allow_html=True,
        )
...
with col_right:
    ...
    if df.empty:
        ...
        if has_market_data:
            ...
            plot_df = plot_df.dropna(subset=required + ["sentiment_score", "density_per_min"])
...
# 3D plot combines sentiment, message density, and market activity
# to show which tickers stand out in the current window.
                fig = px.scatter_3d(
                    plot_df,
                    x="sentiment_score",
                    y="density_per_min",
