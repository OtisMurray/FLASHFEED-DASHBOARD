...
export default function SocialPage() {
  ...
  async function searchTicker(event: FormEvent) {
    ...
    const ticker = tickerSearch.trim().toUpperCase().replace(/[^A-Z0-9.-]/g, '')
    if (!ticker) {
      setTickerFilter('')
      await loadSocial('')
      return
    ...
    try {
      const res = await fetch(`${API_BASE}/api/social/fetch?ticker=${encodeURIComponent(ticker)}`, { method: 'POST' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        setError(data?.error || `Ticker fetch returned HTTP ${res.status}`)
      }
    } catch (err: any) {
